...
def activity_classification_df_builder(people: list[Person], sensors: list[str], activities: list[str]) -> pd.DataFrame:
...
def main(voting: Optional[bool] = False) -> None:
    ...
    if voting == True:
        ...
        vote_metrics_df = pd.DataFrame(vote_metrics, columns=[
                                       "activity", 'specificity', 'sensitivity'])
        vote_metrics_df.loc[len(vote_metrics_df.index)] = [
            'avg', vote_metrics_df['specificity'].mean(), vote_metrics_df['sensitivity'].mean()]
        vote_metrics_df.set_index('activity', inplace=True)
